...
- `product-design` covers broader journey and role decisions; focused screen changes can proceed directly. A styling request needs no prerequisite critique.
...
Resolve missing context only when it affects the requested change; a specific styling correction does not require a new design process.
