...
Promoted to UI_RULES.md: yes — added "section headers use weight 500, never 600+".
```
...
Promoted to UI_RULES.md: yes — added "data-dense surfaces use 36px rows".
```

## The promotion question

After recording every verdict, ask: is this a one-off, or a general rule? If the human's note would apply to other surfaces too ("headers should always...", "we never..."), propose adding it to `UI_RULES.md`. That's the mechanism that makes the human gate reject less over time — each verdict compiles a bit more of the human's taste into the enforceable rules, so the agent gets it right unprompted next time.
